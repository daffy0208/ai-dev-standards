  ...
  getFontUrl(family: string, options?: FontUrlOptions): string {
    ...
    if (options?.weights || options?.styles) {
      ...
      for (const style of styles) {
        const prefix = style === 'italic' ? 'ital,' : ''
        for (const weight of weights) {
          variants.push(style === 'italic' ? `1,${weight}` : `0,${weight}`)
        }
      }

      familyParam += `:${prefix}wght@${variants.join(';')}`
    }
...
<link href="${url}" rel="stylesheet">`
  ...
  async getFontPairings(family: string): Promise<FontPairing[]> {
    ...
    if (font.category === 'serif') {
      pairings.push(
        { heading: family, body: 'Open Sans', reason: 'Classic serif heading with clean sans-serif body', contrast: 'high' },
        { heading: family, body: 'Roboto', reason: 'Traditional serif with modern geometric sans', contrast: 'high' },
        { heading: family, body: 'Lato', reason: 'Elegant serif paired with friendly humanist sans', contrast: 'medium' }
      )
    } else if (font.category === 'sans-serif') {
      pairings.push(
        { heading: family, body: 'Merriweather', reason: 'Modern sans heading with readable serif body', contrast: 'high' },
        { heading: family, body: 'Lora', reason: 'Clean sans with elegant serif for readability', contrast: 'high' },
        { heading: family, body: family, reason: 'Same font family with weight contrast', contrast: 'low' }
      )
    } else if (font.category === 'display') {
      pairings.push(
        { heading: family, body: 'Open Sans', reason: 'Distinctive display with neutral sans body', contrast: 'high' },
        { heading: family, body: 'Roboto', reason: 'Bold display with clean geometric sans', contrast: 'high' }
      )
    } else if (font.category === 'monospace') {
      pairings.push(
        { heading: family, body: 'Inter', reason: 'Technical monospace with professional sans', contrast: 'medium' },
        { heading: 'Inter', body: family, reason: 'Clean heading with monospace for code', contrast: 'medium' }
      )
    }
