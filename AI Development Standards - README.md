...
CI Coverage License: MIT TypeScript Node
...
🎉 Brain/Orchestrator Now Working! The intelligent skill discovery system is now fully configured and documented. Run ./scripts/configure-mcp-paths.sh for one-command setup, then use brain_select_skills to automatically find the right skills for any task. See Brain Quick Start or Complete Solution.

✨ NEW: Code Execution MCP Pattern Implemented! Advanced token-efficient MCP pattern with 40-60% savings (85-95% with skills). Infrastructure ready: Docker sandbox, skills storage, IPython configured. First MCP generated (semantic-search-mcp with 3 tools). See Implementation Complete or Quick Start Guide.
...
CI hardening – Enforce npm run lint, npm run typecheck, npm test, and repolinter on every PR (in-progress).
Code-execution MCP catalog – After the semantic-search pilot is verified, resume the backlog of MCP generators (vector-database, api-validator, deployment-orchestrator, brain).
RAG pipeline + examples – Finish the TypeScript RAG components with integration tests and sample repos.
Template/installer refresh – Revisit the project templates once the CLI commands have automated coverage.
...
INSTALL.md - Legacy setup guide (use INTEGRATION-USAGE.md instead)
UPDATE-GUIDE.md - Legacy update guide (use INTEGRATION-USAGE.md instead)
DOCS/TROUBLESHOOTING.md - Connection and setup issues
For AI Assistants: Loading Context
When working in a project using this repository:

Read META/PROJECT-CONTEXT.md - Understand repository structure and philosophy
Read META/HOW-TO-USE.md - Navigation and usage guide
Read META/DECISION-FRAMEWORK.md - Technology decision guidance
Search META/skill-registry.json - Find relevant skills
...
Layer 3: Decision - Intelligent workflow and tool selection
Layer 4: Management - Strategic planning via Archon MCP
Enabling Automatic Selection
Add project instructions to help Claude use brain tools automatically:

Copy TEMPLATES/claude-instructions-with-brain.md to your project
Rename to .claude/instructions.md
Claude will now follow the skill discovery workflow automatically
Troubleshooting
If the brain isn't working:

Run ./scripts/configure-mcp-paths.sh to fix configuration
Check .claude/mcp-settings.json has correct paths (not hardcoded to another user)
Restart Claude Code / Codex CLI
See DOCS/BRAIN-ORCHESTRATOR-TROUBLESHOOTING.md for detailed help
Common Issue: Hardcoded paths in config files (e.g., /home/david/projects/...) - Run the configuration script to fix.

See:

Quick Reference - Common workflows, examples, cheat sheet
Troubleshooting - Fix common issues
Project Template - Enable automatic brain usage
...
See: Validation-First Development Playbook for the complete 5-phase workflow.
...
See MCP Code Execution Best Practices for detailed guidance
Philosophy
...
Follow official Claude Code YAML frontmatter format
Include comprehensive README with troubleshooting
Test with real use cases before adding
Update META/skill-registry.json
Adding an Architecture Pattern
Must be proven in production environments
Follow problem→solution→trade-offs structure
Include code examples and anti-patterns
Document decision criteria and when to use
Reference related skills and patterns
Updating Existing Content
Propose changes via issue/PR
Explain reasoning and improvements
...
Updated resource counts to match registry (64 skills, 50 MCPs, 238 total resources)
...
FAQ
Q: How is this different from just prompting Claude? A: Skills provide consistent, specialized methodologies automatically. Patterns ensure architectural consistency. You get expert-level guidance without writing detailed prompts every time.

Q: Do I need to memorize the skills? A: No! Claude activates skills automatically based on context. You can also explicitly request: "Use the mvp-builder skill to..."

Q: Can I customize skills for my needs? A: Yes! Skills are meant to evolve. Fork the repository, modify skills, add project-specific skills in .claude/skills/.

Q: What if a skill doesn't fit my use case? A: Skills are focused guidelines, not rigid rules. Explain your context to Claude, and it will adapt the methodology appropriately.

Q: How do I know which pattern to use? A: Check META/DECISION-FRAMEWORK.md for decision trees. Each pattern also includes "When to Use" guidance.

Q: Can I use this with other AI assistants? A: The skills are written for Claude Code's official format, but the patterns and best practices are valuable with any AI assistant.

Q: Is this repository production-ready? A: Yes! All skills and patterns have been tested in real projects. Security and performance best practices are built-in.
...
Existing Projects Guide - Apply to existing codebases ⭐
Sample Project - Try it now with a test project
📝 Templates (Copy & Use)
For New Projects:

Minimal .cursorrules - Basic integration
SaaS .cursorrules - For web applications
AI/RAG .cursorrules - For AI-powered projects
For Existing Projects:

Existing Project .cursorrules - Document and improve existing code ⭐
🎯 By Use Case
Have an existing project? Use existing project template + existing projects guide ⭐
Starting a new SaaS? Use SaaS template + mvp-builder skill
Adding AI search? Use RAG template + rag-implementer skill
Just exploring? Try the sample project
Quick Links
For Humans:
...
Best of both worlds
Documentation: See /DOCS/mcp-patterns/ (~81K words) for complete guides.

Quick Start:

Implementation Complete - ✅ Setup finished, how to use
Quick Start Guide - Get started in 5 minutes
Decision Framework - Which pattern to use?
Code Execution Pattern Guide - Full details
Current State:
