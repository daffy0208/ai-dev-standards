...
import { ErrorTracker, ErrorSeverity, BreadcrumbType } from './error-tracker-tool'

const writeLine = (...messages: unknown[]): void => {
...
function generateReports(): void {
  ...
  // Metrics report
  writeLine('\n=== METRICS REPORT ===')
  writeLine(metrics.exportJSON())

  // Traces report
  writeLine('\n=== TRACES REPORT ===')
  const traces = tracer.getAllTraces()
  writeLine(`Total traces: ${traces.length}`)
  traces.forEach(trace => {
    writeLine(`Trace ${trace.traceId}: ${trace.spans.length} spans`)
  })

  // Errors report
  writeLine('\n=== ERRORS REPORT ===')
  const errorReport = errorTracker.generateReport()
  writeLine(`Total errors: ${errorReport.totalErrors}`)
  writeLine('Top errors:')
  errorReport.topErrors.forEach((error, i) => {
    writeLine(`  ${i + 1}. ${error.message} (${error.count} occurrences)`)
  })

  writeLine('\nBy severity:')
  Object.entries(errorReport.bySeverity).forEach(([severity, count]) => {
    writeLine(`  ${severity}: ${count}`)
  })
}
