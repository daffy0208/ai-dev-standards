...
import neo4j, { Driver, Session, Result, Record as Neo4jRecord } from 'neo4j-driver'
...
export class Neo4jClient {
   ...
   * Create a node
   */
  async createNode(label: string, properties: NodeProperties): Promise<Neo4jRecord> {
    const session = this.getSession()
   ...
   * Delete node
   */
  async deleteNode(label: string, properties: Partial<NodeProperties>): Promise<void> {
    const session = this.getSession()
   ...
   */
  async transaction<T>(work: (session: Session) => Promise<T>): Promise<T> {
    const session = this.getSession()
  ...
  async shortestPath(
    ...
    try {
      ...
      const relPattern = relationshipType ? `[r:${relationshipType}]` : '[r]'
...
export function createNeo4jClient(): Neo4jClient {
  ...
  if (!uri || !username || !password) {
    throw new Error(
      'Neo4j connection variables (NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD) are required'
    )
  }
