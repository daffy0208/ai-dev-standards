...
export interface ToolHandlerConfig<TInput = unknown, TOutput = unknown> {
  name: string
  description: string
  inputSchema: z.ZodSchema
  handler: (args: TInput, context?: ToolExecutionContext) => Promise<TOutput> | TOutput
...
export interface CacheConfig<TInput = unknown> {
  ...
  keyGenerator?: (args: TInput) => string
  enabled?: boolean
}
 ...
 */
class ToolCache<TValue> {
  private cache: Map<string, { value: TValue; expiresAt: number }> = new Map()
 ...
 * - Performance metrics
 */
export class MCPToolHandler<TInput = unknown, TOutput = unknown> {
  private config: Required<ToolHandlerConfig<TInput, TOutput>>
  private cache?: ToolCache<TOutput>
  private rateLimiter?: RateLimiter
  private executionCount: number = 0
  private errorCount: number = 0
  private cacheHits: number = 0

  constructor(config: ToolHandlerConfig<TInput, TOutput>) {
    this.config = {
      ...config,
    ...
    } as Required<ToolHandlerConfig<TInput, TOutput>>

    // Initialize cache if enabled
    if (this.config.cache?.enabled !== false) {
      this.cache = new ToolCache<TOutput>()
    }
  ...
  private async executeWithTimeout(args: TInput, context: ToolExecutionContext): Promise<TOutput> {
    return Promise.race([
      ...
      this.createTimeoutPromise<TOutput>(this.config.timeout)
    ])
  }
   ...
   */
  private validateInput(args: unknown): TInput {
    try {
   ...
   */
  private validateOutput(result: unknown): TOutput {
    if (this.config.validation.outputSchema) {
      try {
        return this.config.validation.outputSchema.parse(result) as TOutput
      } catch (error) {
        if (error instanceof z.ZodError) {
          const errors = error.errors.map(e => `${e.path.join('.')}: ${e.message}`).join(', ')
          throw {
  ...
  private isRetryableError(error: unknown): boolean {
    const typedError = this.toExecutionError(error)
    if (!typedError) {
      return false
    }

    if (!this.config.retries.retryableErrors) {
      return typedError.retryable || false
    }
    return typedError.code ? this.config.retries.retryableErrors.includes(typedError.code) : false
  }
  ...
  /**
   * Cast unknown errors into ToolExecutionError-like shape
   */
  private toExecutionError(error: unknown): Partial<ToolExecutionError> | null {
    if (typeof error === 'object' && error !== null) {
      return error as Partial<ToolExecutionError>
    }
    return null
  }
