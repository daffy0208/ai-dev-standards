...
export interface PromptHandlerConfig {
  name: string
  description: string
  template: string | ((args: PromptArgs) => string | Promise<string>)
...
export interface PromptVariable {
  ...
  required?: boolean
  default?: unknown
  validation?: z.ZodSchema | ((value: unknown) => boolean)
}
...
export class MCPPromptHandler {
   ...
   */
  async execute(args: PromptArgs = {}): Promise<PromptExecutionResult> {
    ...
    try {
      ...
      return this.createErrorResult(
        (err.code as string) || 'PROMPT_EXECUTION_FAILED',
        (err.message as string) || 'Failed to execute prompt',
        args,
        startTime,
        err.details
      )
   ...
   */
  private processVariables(args: PromptArgs): PromptArgs {
    const processed: PromptArgs = {}
   ...
   */
  private validateVariableType(name: string, value: unknown, variable: PromptVariable): void {
   ...
   */
  private validateVariableValue(name: string, value: unknown, variable: PromptVariable): void {
   ...
   */
  private substituteVariables(template: string, args: PromptArgs): string {
    let result = template
   ...
   * Get cache key
   */
  private getCacheKey(args: PromptArgs): string {
