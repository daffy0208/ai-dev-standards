...
function createContextPaths(baseDir) {
  const CONTEXT_DIR = path.join(baseDir, '.claude', 'context')
  return {
    CONTEXT_DIR,
    FILES_DIR: path.join(CONTEXT_DIR, 'files'),
    SESSIONS_DIR: path.join(CONTEXT_DIR, 'sessions'),
    ACTIVE_SESSION_FILE: path.join(CONTEXT_DIR, 'active-session.json')
  }
}
 ...
 */
function showContext(deps = defaultDeps) {
  const { fs, chalk } = deps
  const { ACTIVE_SESSION_FILE } = createContextPaths(deps.cwd())
  ...
  console.log(
    chalk.gray(
      `Started: ${getTimeAgo(session.startTime)} (${new Date(session.startTime).toLocaleString()})`
    )
  )
  ...
  if (session.modifiedFiles && session.modifiedFiles.length > 0) {
    ...
    recentFiles.forEach((file, index) => {
      ...
      const typeColor =
        file.type === 'added' ? chalk.green : file.type === 'deleted' ? chalk.red : chalk.yellow
 ...
 */
function showHistory(options = {}, deps = defaultDeps) {
  const { fs, chalk } = deps
  const { SESSIONS_DIR } = createContextPaths(deps.cwd())
  ...
  sessionFiles.forEach((file, index) => {
    ...
    console.log(
      chalk.gray(
        `   Activity: ${session.promptCount || 0} prompts, ${session.filesModified || 0} files modified`
      )
    )
 ...
 */
function clearContext(options = {}, deps = defaultDeps) {
  const { fs, chalk, path } = deps
  const { FILES_DIR, SESSIONS_DIR } = createContextPaths(deps.cwd())
  ...
  if (fs.existsSync(SESSIONS_DIR)) {
    ...
    for (const sessionFile of sessions) {
      ...
      try {
        ...
        // Ignore errors when deleting session files
      }
    }
  }
 ...
 */
function restoreSession(sessionId, deps = defaultDeps) {
  const { fs, path, chalk } = deps
  const { SESSIONS_DIR, ACTIVE_SESSION_FILE } = createContextPaths(deps.cwd())
 ...
 */
function showStats(deps = defaultDeps) {
  const { fs, path, chalk } = deps
  const { FILES_DIR, SESSIONS_DIR, ACTIVE_SESSION_FILE } = createContextPaths(deps.cwd())
  console.log(chalk.bold.cyan('\n📊 Context Statistics\n'))
 ...
 */
function contextCommand(action, options = {}, deps = defaultDeps) {
  switch (action) {
    case 'show':
      showContext(deps)
      break
    ...
    case 'history':
      showHistory(options, deps)
      break

    case 'clear':
      clearContext(options, deps)
      break

    case 'restore':
      if (!options.sessionId) {
        console.log(deps.chalk.red('\n✗ Session ID required\n'))
        console.log(deps.chalk.gray('Usage: ai-dev context restore <session-id>\n'))
        return
      }
      restoreSession(options.sessionId, deps)
      break
    ...
    default:
      ...
      console.log(deps.chalk.gray('Available actions: show, history, clear, restore, stats\n'))
  }
}

module.exports = contextCommand
module.exports.showContext = showContext
module.exports.showHistory = showHistory
module.exports.clearContext = clearContext
module.exports.restoreSession = restoreSession
module.exports.showStats = showStats
module.exports.createContextPaths = createContextPaths
