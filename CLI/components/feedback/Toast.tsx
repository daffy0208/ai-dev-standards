...
const toastEventTarget = typeof window === 'undefined' ? undefined : window
 ...
 */
const dispatchToast = (detail: Omit<Toast, 'id'>) => {
  if (!toastEventTarget) return
  toastEventTarget.dispatchEvent(new CustomEvent(toastEventName, { detail }))
}

export const toast = {
  success: (message: string, description?: string, duration?: number) =>
    dispatchToast({ type: 'success', message, description, duration }),
  error: (message: string, description?: string, duration?: number) =>
    dispatchToast({ type: 'error', message, description, duration }),
  warning: (message: string, description?: string, duration?: number) =>
    dispatchToast({ type: 'warning', message, description, duration }),
  info: (message: string, description?: string, duration?: number) =>
    dispatchToast({ type: 'info', message, description, duration })
}
...
export function ToastProviderWithEvents({
  ...
  useEffect(() => {
    ...
    toastEventTarget.addEventListener(toastEventName, handleToast)
    return () => toastEventTarget.removeEventListener(toastEventName, handleToast)
  }, [addToast])
