      ...
      <div className="flex items-start">
        ...
        <div className="ml-3 flex-1">
          ...
          <div className="mt-2 text-sm text-orange-700">
            <p>
              You don&apos;t have permission to access {resource || 'this resource'}. Please contact
...
export function InlineError({ error, onRetry }: ErrorFallbackProps) {
  ...
  return (
    <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 rounded px-3 py-2">
      ...
      {onRetry && (
        <button onClick={onRetry} className="text-red-700 hover:text-red-800 font-medium underline">
          Retry
        </button>
      )}
    </div>
...
export function Examples() {
  return (
    <div className="space-y-6 p-6">
      <ErrorFallback error={new Error('Something went wrong')} onRetry={() => Promise.resolve()} />

      <APIError error={new Error('Unauthorized')} statusCode={401} />

      <NetworkError onRetry={() => Promise.resolve()} />

      <PermissionError resource="admin dashboard" />

      <LoadingError onRetry={() => Promise.resolve()} />

      <InlineError error="Invalid email format" onRetry={() => Promise.resolve()} />

      <EmptyState
        title="No messages"
        message="You don't have any messages yet"
        action={
          <button className="px-4 py-2 bg-blue-600 text-white rounded-lg">Compose Message</button>
        }
      />
    </div>
  )
