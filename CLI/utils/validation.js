...
function sanitizeName(name, resourceType = 'resource') {
  ...
  if (trimmed.includes('..')) {
    throw new Error(
      `Invalid ${resourceType} name: parent directory reference (..) is not allowed`
    );
  }
  ...
  if (trimmed.startsWith('.')) {
    throw new Error(
      `Invalid ${resourceType} name: names cannot start with a dot (hidden files)`
    );
  }
  ...
  if (!/^[a-zA-Z]/.test(trimmed)) {
    throw new Error(
      `Invalid ${resourceType} name: must start with a letter (a-z or A-Z)`
    );
  }
...
function validateIdentifier(identifier, context = 'identifier') {
  ...
  if (reservedKeywords.includes(trimmed.toLowerCase())) {
    throw new Error(
      `Invalid ${context}: "${trimmed}" is a reserved JavaScript keyword`
    );
  }
...
function validateFramework(framework, allowedFrameworks) {
  if (!framework || typeof framework !== 'string') {
    throw new Error(
      `Framework is required. Allowed values: ${allowedFrameworks.join(', ')}`
    );
  }
  ...
  if (!allowedFrameworks.includes(trimmed)) {
    throw new Error(
      ...
      `Allowed values: ${allowedFrameworks.join(', ')}`
    );
  }
...
function validateProjectType(type, allowedTypes) {
  if (!type || typeof type !== 'string') {
    throw new Error(
      `Project type is required. Allowed values: ${allowedTypes.join(', ')}`
    );
  }
...
function validatePythonIdentifier(identifier, context = 'identifier') {
  ...
  if (pythonKeywords.includes(trimmed)) {
    throw new Error(
      `Invalid Python ${context}: "${trimmed}" is a reserved Python keyword`
    );
  }
