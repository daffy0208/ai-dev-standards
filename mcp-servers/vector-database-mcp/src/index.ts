...
class VectorDatabaseServer {
  ...
  private async handleSearchVectors(args: { query: number[]; topK?: number; filter?: any }) {
    if (!this.client) {
      throw new Error('Not connected. Call connect() first.')
    }

    const results = await this.client.searchVectors(args.query, args.topK || 10, args.filter)
