        ...
        inputSchema: {
          ...
          properties: {
            ...
            depth: {
              ...
              description:
                'Analysis depth: quick (basic metrics), medium (patterns), deep (full analysis)',
              default: 'medium'
            },
            include_patterns: {
              type: 'array',
  ...
  private async scanPatterns(repoPath: string, depth: string): Promise<Pattern[]> {
    ...
    if (this.metrics.doc_files > 20 && this.metrics.avg_doc_length > 400) {
      patterns.push({
        ...
        interpretation:
          'Planning activity may significantly outpace execution. High documentation volume suggests over-planning or avoidance through writing.',
        confidence: 0.85,
        evidence: [
    ...
    if (this.metrics.doc_to_code_ratio > 0.5) {
      patterns.push({
        ...
        interpretation:
          'More documentation than implementation suggests hope-driven development. System may be aspirational rather than actionable.',
        confidence: 0.9,
        evidence: [
    ...
    if (this.metrics.technical_debt_markers > 50) {
      patterns.push({
        ...
        interpretation:
          'High technical debt markers suggest time pressure, fatigue, or incomplete implementations. May indicate rushing or scope creep.',
        confidence: 0.75,
        evidence: [
    ...
    const testCoverage =
      this.metrics.code_files > 0 ? this.metrics.test_files / this.metrics.code_files : 0
    ...
    if (testCoverage < 0.3 && this.metrics.code_files > 10) {
      patterns.push({
        ...
        interpretation:
          'Low test coverage may indicate rushed implementation or avoidance of validation feedback.',
        confidence: 0.7,
        evidence: [
...
`
  ...
  private generateRecommendations(rci: RCIScore, patterns: Pattern[]): string {
    ...
    if (critical.length > 0 || rci.overall < 50) {
      ...
      if (rci.overall < 50) {
        recommendations +=
          '**Action:** Pause development. Reconcile intent with reality before proceeding.\n\n'
      }
    }
    ...
    return (
      recommendations || '**No critical issues detected.** Continue current rhythm with awareness.'
    )
  }
  ...
  private async handleAnalyzeDocumentation(args: any) {
    ...
    const docFiles = (await this.walkDirectory(docPath, excludePatterns)).filter(f =>
      /\.md$/i.test(f)
    )
  ...
  private async handleCheckCoherence(args: any) {
    ...
    const warning =
      checks.readme_has_todos && checks.claims_completeness
