...
const tools: Tool[] = [
  {
    ...
    description:
      'Create a new knowledge base entry. Supports document-based (RAG) and entity-based (graph) knowledge.',
    inputSchema: {
      type: 'object',
      properties: {
        type: {
  ...
  },
  {
    ...
    description:
      'Update an existing knowledge entry. Creates a new version while preserving history.',
    inputSchema: {
      type: 'object',
      properties: {
        id: {
  ...
  },
  {
    ...
    description:
      'Delete a knowledge entry. Can be soft delete (mark as deleted) or hard delete (remove completely).',
    inputSchema: {
      type: 'object',
      properties: {
        id: {
        ...
        hard_delete: {
          type: 'boolean',
          description:
            'If true, permanently deletes. If false, marks as deleted but preserves history.'
        }
      },
      required: ['id']
    }
  },
  {
    ...
    description:
      'Query the knowledge base. Supports filtering by type, category, tags, and text search.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  },
  {
    ...
    description:
      'Validate knowledge for duplicates, conflicts, and quality issues. Returns validation report.',
    inputSchema: {
      type: 'object',
      properties: {
        id: {
  ...
  },
  {
    ...
    description:
      'Bulk import knowledge entries from JSON. Useful for migrating or seeding knowledge base.',
    inputSchema: {
      type: 'object',
      properties: {
        entries: {
...
async function handleQueryKnowledge(args: any): Promise<any> {
  ...
  for (const [id, versions] of knowledgeBase.entries) {
    ...
    if (args.tags && !args.tags.some((tag: string) => current.metadata.tags?.includes(tag)))
      continue
  ...
  for (const entry of entriesToCheck) {
    ...
    if (checks.includes('staleness')) {
      const daysSinceUpdate =
        (Date.now() - new Date(entry.metadata.updated_at).getTime()) / (1000 * 60 * 60 * 24)
  ...
  for (const [id, versions] of knowledgeBase.entries) {
    ...
    for (const entry of entriesToExport) {
      ...
      if (args.filters) {
        ...
        if (
          args.filters.tags &&
          !args.filters.tags.some((tag: string) => entry.metadata.tags?.includes(tag))
        )
          continue
  ...
  try {
    ...
    switch (name) {
      case 'create_knowledge_entry':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleCreateKnowledgeEntry(args), null, 2) }
          ]
        }
      case 'update_knowledge_entry':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleUpdateKnowledgeEntry(args), null, 2) }
          ]
        }
      case 'delete_knowledge_entry':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleDeleteKnowledgeEntry(args), null, 2) }
          ]
        }
      case 'query_knowledge':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleQueryKnowledge(args), null, 2) }
          ]
        }
      case 'validate_knowledge':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleValidateKnowledge(args), null, 2) }
          ]
        }
      case 'version_knowledge':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleVersionKnowledge(args), null, 2) }
          ]
        }
      case 'import_knowledge':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleImportKnowledge(args), null, 2) }
          ]
        }
      case 'export_knowledge':
        return {
          content: [
            { type: 'text', text: JSON.stringify(await handleExportKnowledge(args), null, 2) }
          ]
        }
      default:
        return {
