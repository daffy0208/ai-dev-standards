...
function extractMCPMetadata(mcpName: string): MCPMetadata | null {
  ...
  let name = mcpName.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      if (pkg.name) {
        name = pkg.name.replace(/-mcp$/, '').split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') + ' MCP';
      }
    } catch (e) {
...
function generateMCPRegistry(): void {
  ...
  const mcpDirs = fs.readdirSync(mcpDir)
    .filter(dir => {
  ...
  const registry = {
    version: '1.0.3',
    last_updated: new Date().toISOString().split('T')[0],
    description: 'Registry of all MCP servers in ai-dev-standards. MCPs (Model Context Protocol servers) provide tools, resources, and prompts that skills can use to execute tasks.',
...
function generateToolRegistry(): void {
  ...
  let toolCount = 0;
  const categories = ['custom-tools', 'crewai-tools', 'langchain-tools', 'mcp-tools', 'observability', 'media-tools', 'design-tools'];
  
  for (const category of categories) {
    const categoryPath = path.join(toolsDir, category);
    if (fs.existsSync(categoryPath)) {
      const files = fs.readdirSync(categoryPath).filter(f => f.endsWith('.ts') && !f.includes('test'));
      toolCount += files.length;
    }
  }

  console.log(`${GREEN}✅ Counted tools: ${toolCount} tools${RESET}`);
  console.log(`${YELLOW}   Note: tool-registry.json is manually maintained with detailed metadata${RESET}`);
}
...
function generateComponentRegistry(): void {
  ...
  console.log(`${YELLOW}   Note: component-registry.json is manually maintained with detailed metadata${RESET}`);
}
...
function generateIntegrationRegistry(): void {
  ...
  console.log(`${YELLOW}   Note: integration-registry.json is manually maintained with detailed metadata${RESET}`);
}
