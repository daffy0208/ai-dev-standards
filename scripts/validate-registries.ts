...
interface SkillRegistry extends Record<string, unknown> {
  total_skills: number
...
interface McpRegistry extends Record<string, unknown> {
  total_mcps: number
...
interface ToolRegistry extends Record<string, unknown> {
  total_tools: number
...
interface ComponentRegistry extends Record<string, unknown> {
...
interface IntegrationRegistry extends Record<string, unknown> {
...
function validateSkillRegistry(): ValidationResult {
  ...
  const registry = loadRegistry<SkillRegistry>('skill-registry.json', 'skills', result)
  if (!registry) {
    return result
  }
  ...
  const actualSkills = fs.readdirSync(skillsDir).filter(dir => {
  ...
  if (result.passed) {
    console.log(
      `${GREEN}✅ skill-registry.json is valid (${registry.skills?.length || 0} skills)${RESET}`
    )
  }
...
function validateMCPRegistry(): ValidationResult {
  ...
  const registry = loadRegistry<McpRegistry>('mcp-registry.json', 'mcps', result)
  if (!registry) {
    return result
  }
  ...
  const actualMCPs = fs.readdirSync(mcpDir).filter(dir => {
  ...
  // Check for orphaned registry entries
  if (registry.mcps) {
    const registryIds = new Set(registry.mcps.map(mcp => mcp.id))
  ...
  // Check for duplicate IDs
  if (registry.mcps) {
    const ids = registry.mcps.map(mcp => mcp.id)
  ...
  if (result.passed) {
    console.log(
      `${GREEN}✅ mcp-registry.json is valid (${registry.mcps?.length || 0} MCPs)${RESET}`
    )
  }
...
function validateToolRegistry(): ValidationResult {
  ...
  const registry = loadRegistry<ToolRegistry>('tool-registry.json', 'tools', result)
  if (!registry) {
    return result
  }
  ...
  if (result.passed) {
    console.log(
      `${GREEN}✅ tool-registry.json is valid (${registry.tools?.length || 0} tools)${RESET}`
    )
  }
...
function validateComponentRegistry(): ValidationResult {
  ...
  const registry = loadRegistry<ComponentRegistry>('component-registry.json', 'components', result)
  if (!registry) {
    return result
  }
  ...
  if (result.passed) {
    console.log(
      `${GREEN}✅ component-registry.json is valid (${registry.components?.length || 0} components)${RESET}`
    )
  }
...
function validateIntegrationRegistry(): ValidationResult {
  ...
  const registry = loadRegistry<IntegrationRegistry>(
  ...
  if (result.passed) {
    console.log(
      `${GREEN}✅ integration-registry.json is valid (${registry.integrations?.length || 0} integrations)${RESET}`
    )
  }
...
main()
function loadRegistry<T extends Record<string, unknown>>(
  fileName: string,
  key: string,
  result: ValidationResult
): T | null {
  const registryPath = path.join(ROOT, 'META', fileName)
  ...
  try {
    const parsed = JSON.parse(fs.readFileSync(registryPath, 'utf-8')) as T
    const section = (parsed as Record<string, unknown>)[key]
    if (!Array.isArray(section)) {
      result.errors.push(`${fileName} missing "${key}" array`)
      result.passed = false
      return null
    }
    return parsed
  } catch (e) {
    result.errors.push(`${fileName} is not valid JSON: ${e}`)
    result.passed = false
    return null
  }
}
