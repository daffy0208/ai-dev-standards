...
async function commandStatus(brain: RepositoryBrain): Promise<void> {
  ...
  const healthColor =
    status.health === 'healthy' ? 'green' : status.health === 'degraded' ? 'yellow' : 'red'
...
async function commandReverseDeps(
...
async function commandPatterns(brain: RepositoryBrain, problem: string): Promise<void> {
  ...
  matches.slice(0, 3).forEach((match: PatternMatch, i: number) => {
    ...
    console.log(
      `   ${colorize('Complexity:', 'yellow')} ${match.pattern.complexity} | ${colorize('Time:', 'yellow')} ${match.pattern.estimated_time}`
    )
    ...
    // Show pros/cons
    console.log(
      `   ${colorize('Pros:', 'green')} ${match.pattern.tradeoffs.pros.slice(0, 2).join(', ')}`
    )
    console.log(
      `   ${colorize('Cons:', 'red')} ${match.pattern.tradeoffs.cons.slice(0, 2).join(', ')}`
    )

    // Show required skills
    if (match.pattern.skills.length > 0) {
      console.log(
        `   ${colorize('Skills:', 'cyan')} ${match.pattern.skills.slice(0, 3).join(', ')}`
      )
    }
  })
...
async function commandWorkflow(brain: RepositoryBrain, scenario: string): Promise<void> {
  ...
  workflow.steps.forEach((step: WorkflowStep) => {
    ...
    console.log(
      `     ${colorize('Time:', 'yellow')} ${step.estimatedTime} | ${colorize('Skills:', 'cyan')} ${step.requiredSkills.join(', ')}`
    )
    console.log()
  })
...
async function commandAnalyze(brain: RepositoryBrain, task: string): Promise<void> {
  ...
  console.log(
    `${colorize('Recommended Skills:', 'cyan')} ${analysis.complexity.recommendedSkillCount}`
  )
  console.log(
    `${colorize('Total Estimated Time:', 'yellow')} ${analysis.summary.totalEstimatedTime}`
  )
