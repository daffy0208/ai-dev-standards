...
export class ApproachSelector {
  ...
  async selectApproach(
    ...
    const codeExecutionReasons: string[] = []
    if (factors.complexity_score >= this.config.complexity_threshold) {
      codeExecutionReasons.push(`Complexity score ${factors.complexity_score} exceeds threshold`)
    }
    if (factors.estimated_tools >= Math.max(3, this.config.tools_threshold - 1)) {
      codeExecutionReasons.push(`Multiple systems involved (${factors.estimated_tools} tools)`)
    }
    if (factors.data_size_kb >= this.config.data_size_threshold_kb) {
      codeExecutionReasons.push(`Large data payload: ${factors.data_size_kb}KB`)
    }
    if (factors.has_pii) {
      codeExecutionReasons.push('PII present - needs secure execution environment')
    }
    if (factors.is_repeated) {
      codeExecutionReasons.push('Repeated workflow benefits from reusable skills')
    }
    if (factors.workflow_type === 'complex') {
      codeExecutionReasons.push('Complex branching workflow detected')
    }

    if (codeExecutionReasons.length > 0) {
      codeExecutionReasons.unshift(`Complexity score: ${factors.complexity_score}/10`)
      return this.createDecision('code-execution', factors, 0.85, codeExecutionReasons)
    }
  ...
  private createDecision(
    ...
    const normalized = (reasoning ?? []).filter(Boolean) as string[]
    const hasComplexityReason = normalized.some(reason => reason.toLowerCase().includes('complex'))
    if (!hasComplexityReason) {
      normalized.push(`Complexity score: ${factors.complexity_score}/10`)
    }
