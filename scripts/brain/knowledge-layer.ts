...
export class KnowledgeLayer {
  ...
  searchSkills(query: string): Skill[] {
    ...
    return this.skillRegistry.skills.filter(
      s =>
  ...
  searchMCPs(query: string): MCP[] {
    if (!this.mcpRegistry) throw new Error('Registries not loaded')
    const lowerQuery = query.toLowerCase()
    return this.mcpRegistry.mcps.filter(
      m =>
  ...
  getTool(idOrName: string): Tool | null {
    ...
    const lowerQuery = idOrName.toLowerCase()
    return (
      this.toolRegistry.tools.find(
        tool => tool.id === idOrName || tool.name?.toLowerCase() === lowerQuery
      ) || null
    )
  }
  ...
  getComponent(idOrName: string): Component | null {
    ...
    const lowerQuery = idOrName.toLowerCase()
    return (
      this.componentRegistry.components.find(
        component => component.id === idOrName || component.name?.toLowerCase() === lowerQuery
      ) || null
    )
  }
  ...
  getIntegration(idOrName: string): Integration | null {
    ...
    const lowerQuery = idOrName.toLowerCase()
    return (
      this.integrationRegistry.integrations.find(
        integration => integration.id === idOrName || integration.name?.toLowerCase() === lowerQuery
      ) || null
    )
  }
      ...
      for (const [skillName, deps] of Object.entries(this.relationshipMapping.skills)) {
        for (const mcpName of deps.required_mcps) {
          if (!this.getMCP(mcpName)) {
            errors.push(
              `Required MCP '${mcpName}' for skill '${skillName}' not found in MCP registry`
            )
          }
        }
        ...
        for (const toolName of deps.required_tools) {
          if (!this.getTool(toolName)) {
            errors.push(
              `Required tool '${toolName}' for skill '${skillName}' not found in tool registry`
            )
          }
        }
        ...
        for (const componentName of deps.required_components) {
          if (!this.getComponent(componentName)) {
            errors.push(
              `Required component '${componentName}' for skill '${skillName}' not found in component registry`
            )
          }
        }
        ...
        for (const integrationName of deps.required_integrations) {
          if (!this.getIntegration(integrationName)) {
            errors.push(
              `Required integration '${integrationName}' for skill '${skillName}' not found in integration registry`
            )
          }
        }
      }
