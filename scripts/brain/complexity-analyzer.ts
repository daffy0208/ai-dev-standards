...
export class ComplexityAnalyzer {
  ...
  async analyzeComplexity(taskDescription: string): Promise<ComplexityScore> {
    ...
    const overallScore =
      Math.round(
        (toolsScore * 0.3 + stepsScore * 0.25 + dataScore * 0.25 + logicScore * 0.2) * 10
      ) / 10

    // Determine workflow structure
    const workflowStructure = this.determineWorkflowStructure(stepsAnalysis, logicAnalysis)

    // Build reasoning
    const reasoning: string[] = []
    reasoning.push(
      `Estimated ${toolsAnalysis.count} tool(s) needed: ${toolsAnalysis.tools.join(', ')}`
    )
  ...
  private analyzeSteps(text: string): { count: number; is_sequential: boolean } {
    ...
    const sequenceMarkers = text.match(
      /\b(first|second|third|fourth|fifth|finally|lastly|initially|subsequently|afterwards?)\b/gi
    )
    if (sequenceMarkers) {
      const sequenceSteps = Math.min(sequenceMarkers.length + 1, 10)
      stepCount = Math.max(stepCount, sequenceSteps)
    }
    ...
    const actionVerbs = text.match(
      /\b(get|create|update|delete|send|analyze|transform|process|validate|check|generate|build|copy|post|fetch|save|upload|download|summarize|review|plan|design)\b/gi
    )
    ...
    const isSequential =
      /(then|next|after|followed by|sequential|step-by-step|first|second|third|finally)/i.test(text)
  ...
  private analyzeDataComplexity(text: string): {
    ...
    const transformPatterns = [
      ...
      /\b(extract|filter|aggregate|merge|join|combine|analyz(e|ing)|summarize)\b/i,
    ...
    return {
      ...
      transformation_needed: transformationNeeded
    }
  }
  ...
  private calculateStepsScore(stepCount: number): number {
    ...
    if (stepCount === 4) return 7
    return Math.min(7 + stepCount - 4, 10)
  }
  ...
  private calculateDataScore(dataAnalysis: ReturnType<typeof this.analyzeDataComplexity>): number {
    ...
    if (dataAnalysis.complexity === 'moderate') return 6
    return 9
  }
