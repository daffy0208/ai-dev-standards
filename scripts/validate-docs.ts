...
function loadRegistryCounts(): RegistryCounts {
  ...
  const integrations = integrationRegistry.total_integrations || integrationRegistry.integrations?.length || 0;
...
function main(): void {
  ...
  try {
    ...
    if (allErrors.length === 0) {
      ...
      if (allWarnings.length > 0) {
        console.log(`${YELLOW}   (${allWarnings.length} warnings - some files missing AUTO-GEN markers)${RESET}`);
