...
function validateMCPSettings(): ValidationResult {
  ...
  try {
    settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8')) as McpSettings
  } catch (e) {
  ...
  for (const [mcpId, mcpConfig] of Object.entries(settings.mcpServers)) {
    // Check command
 ...
 */
function checkRequiredMCPs(settings: McpSettings): ValidationResult {
  const result: ValidationResult = {
    passed: true,
    errors: [],
    warnings: []
...
function main(): void {
  ...
  try {
    ...
    const settingsPath = path.join(ROOT, '.claude', 'mcp-settings.json')
    const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8')) as McpSettings
    const requiredResult = checkRequiredMCPs(settings)
